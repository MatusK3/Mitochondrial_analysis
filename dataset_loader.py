...
class Dataset():
    def __init__(self, dataset_paths :dict):
        ...
        self.load_data(dataset_paths["path"], dataset_paths["structure_path"], dataset_paths["data_types"], dataset_paths["segmentation_masks_path"])

    
    def load_data(self, data_path: Path, dataset_structure_path: Path, data_type :DATASET_TYPES, segmentation_masks_path :Path):
        ...
        for scene_id in df['scene'].unique():
            ...
            self.scenes.append(get_scene(data_type, self.data_path, scans, segmentation_masks_path))
