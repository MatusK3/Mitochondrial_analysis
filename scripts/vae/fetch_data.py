...
class FlattenedROIDataset(Dataset):
    def __init__(self, csv_path: str, config_path: str, data_dir: str, split_name: str="train", transform=None):
        ...
        if type(split_name) is list:
            allowed_scenes = []
            for i in split_name:
                allowed_scenes.extend(config["dataset_split"][i])
        else:
    ...
    def __getitem__(self, idx):
        ...
        img_tensor = torch.from_numpy(roi)
        if self.transform:
    ...
    def get_sampler(self):
        target_labels = np.array([sample['label'] for sample in self.roi_samples])

        # 3. Calculate class weights
        class_sample_count = np.array([len(np.where(target_labels == t)[0]) for t in np.unique(target_labels)])
        weight = 1. / class_sample_count
        samples_weight = torch.from_numpy(np.array([weight[t] for t in target_labels]))

        # 4. Create the Sampler
        return WeightedRandomSampler(weights=samples_weight, num_samples=len(samples_weight), replacement=True)
...
transform_train = T.Compose([
    ...
    # T.RandomErasing(p=0.5, scale=(0.02, 0.2), value=0.0),
])
...
def fetch_train_val_dataloaders(batch_size: int=64, transform=transform_train) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    ...
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, num_workers=3, sampler = trainset.get_sampler())
    val_loader = torch.utils.data.DataLoader(valset, batch_size=batch_size, sampler = valset.get_sampler())

    return train_loader, val_loader



def fetch_big_dataloader(batch_size: int=64, transform=transform_train) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    trainset = fetch_ROI_dataset(["train", "val"], transformation=transform)
    # train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True)
    # val_loader = torch.utils.data.DataLoader(valset, batch_size=batch_size, shuffle=False)
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, num_workers=3, sampler = trainset.get_sampler())
...
def fetch_mini(num_of_samples: int = 32, random_seed=42) -> torch.utils.data.DataLoader:
    dataset = fetch_ROI_dataset("train", transformation=transform_test)
    ...
    mini_loader = torch.utils.data.DataLoader(mini,batch_size=num_of_samples,shuffle=False)
    return mini_loader
